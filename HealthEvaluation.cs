...
public sealed record HealthEvaluation(HealthStatus Status, string? Reason = null)
{
    ...
    /// <summary>
    /// Creates a not-yet-determined evaluation with the given reason ("awaiting first probe").
    /// <c>Unknown</c> is expected to be transient — by convention, not enforcement; see ADR-008
    /// (docs/adr/008-unknown-must-be-transient.md).
    /// </summary>
    public static HealthEvaluation Unknown(string reason) => new(HealthStatus.Unknown, reason);
