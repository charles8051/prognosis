...
public class DependsOnEdgeAnalyzerTests
{
    ...
    private const string Stubs = """
        namespace Prognosis
        {
            ...
            public sealed class HealthNode
            {
                ...
                public static (HealthNode Node, object Lease) CreateLeased(string name, object options)
                    => (new HealthNode(), new object());
            }
    ...
    [Fact]
    public async Task CreateLeased_RegistersValidReference()
    {
        var source = Stubs + """
            ...
            class Setup
            {
                void Configure()
                {
                    Prognosis.HealthNode.CreateLeased("Database", new object());
                    var configurator = new Prognosis.DependencyInjection.DependencyConfigurator();
                    configurator.DependsOn("Database", Prognosis.Importance.Required);
                }
            }
