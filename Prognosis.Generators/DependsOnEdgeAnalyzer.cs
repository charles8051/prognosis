...
/// <c>HealthNode</c> factory methods and
...
public sealed class DependsOnEdgeAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly DiagnosticDescriptor s_unknownNodeRule = new(
        ...
        messageFormat: "Node name '{0}' does not match any HealthNode factory or PrognosisBuilder.AddNode call in this compilation",
        category: "Prognosis",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
        description:
            "The referenced node name was not found among names passed to " +
            "a HealthNode factory or PrognosisBuilder.AddNode in the " +
        ...
        ConcurrentDictionary<string, bool> knownNames)
    {
        ...
        if (methodName is not ("Create" or "CreateLeased" or "CreateDelegate" or "CreateComposite" or "AddNode"))
            return;
        ...
        var isHealthNode = method.ContainingType is { Name: "HealthNode" }
            ...
            && method.Name is "Create" or "CreateLeased" or "CreateDelegate" or "CreateComposite";
