...
public sealed class HealthNodeNameCollector : IIncrementalGenerator
{
    ...
    /// <c>CreateLeased</c>, <c>CreateDelegate</c>, <c>CreateComposite</c>,
    /// or <c>AddNode</c>
    ...
    private static bool IsHealthNodeFactoryCandidate(SyntaxNode node)
    {
        ...
        return name is "Create" or "CreateLeased" or "CreateDelegate" or "CreateComposite" or "AddNode"
    ...
    private static string? ExtractNodeName(GeneratorSyntaxContext ctx)
    {
        ...
        else if (methodName is not ("AddNode"
                                 or "Create" or "CreateLeased" or "CreateDelegate" or "CreateComposite"))
        {
            return null;
        }
    ...
    private static string GenerateSource(ImmutableArray<string> names)
    {
        ...
        sb.AppendLineLf();
        sb.AppendLineLf("namespace Prognosis;");
        sb.AppendLineLf();
        sb.AppendLineLf("/// <summary>");
        sb.AppendLineLf("/// Auto-generated constants for every health node name discovered in this");
        sb.AppendLineLf("/// compilation via <c>HealthNode</c> factory methods and");
        sb.AppendLineLf("/// <c>PrognosisBuilder.AddNode</c> calls.");
        sb.AppendLineLf("/// Use these constants in place of string literals for refactoring safety,");
        sb.AppendLineLf("/// autocomplete, and find-all-references support.");
        sb.AppendLineLf("/// </summary>");
        sb.AppendLineLf("public static class HealthNames");
        sb.AppendLineLf("{");

        foreach (var name in names)
        {
            var fieldName = SanitizeFieldName(name);
            sb.AppendLineLf($"    /// <summary>Node name: <c>\"{EscapeXml(name)}\"</c></summary>");
            sb.AppendLineLf($"    public const string {fieldName} = \"{EscapeString(name)}\";");
        }
