...
public class AggregationTests
{
    ...
    // ADR-006: Unknown is strictly non-gating. Optional ignores the child entirely,
    // so an Unknown child leaves the parent Healthy; Resilient passes Unknown through
    // (the Degraded cap fires only on an Unhealthy sibling), so the parent is at most
    // Unknown — never Degraded/Unhealthy. Completes Importance x Unknown coverage.
    [InlineData(HealthStatus.Unknown, Importance.Optional, HealthStatus.Healthy)]
    [InlineData(HealthStatus.Unknown, Importance.Resilient, HealthStatus.Unknown)]
    // ADR-008: Advisory is Important that ABSORBS Unknown. A failure still degrades the
    // parent (capped, never Unhealthy); an indeterminate advisory child leaves it Healthy.
    // This is the row that distinguishes Advisory from every other importance.
    [InlineData(HealthStatus.Unknown, Importance.Advisory, HealthStatus.Healthy)]
    [InlineData(HealthStatus.Unhealthy, Importance.Advisory, HealthStatus.Degraded)]
    [InlineData(HealthStatus.Degraded, Importance.Advisory, HealthStatus.Degraded)]
    [InlineData(HealthStatus.Healthy, Importance.Advisory, HealthStatus.Healthy)]
