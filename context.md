...
- **`Importance`** — enum: `Required`, `Important`, `Optional`, `Resilient`, `Advisory`. Controls how dependency failures propagate.
...
| `Advisory` | Like `Important`, but `Unknown` is absorbed (contributes `Healthy`) — ADR-008 |
...
- **Reason chains** — `Aggregate` nests the worst dependency's reason as `"dep: <its reason>"`, forming a path from the node to the culprit. A back edge (a dependency still on the wave's DFS stack) is cut to the flat `"dep is <status>"` form, which embeds a status rather than a nested chain and so terminates the chain. Each node appears at most once as a hop and length is bounded by walk depth; the terminal *may* repeat a hop, which is what closing a cycle looks like (`"C: B: A is Unhealthy"` reported by A). Without the cut a cycle accumulates one lap per wave, and since `Reason` is part of the report-equality key (ADR-012 §1) the graph then re-emits on every beat forever. Normative in ADR-012 §5 as amended 2026-08-22. Both walkers (`NotifyDfs`, `EvalInDependencyOrder`) carry the gray set for this; it is inert on acyclic graphs.
