...
├── Prognosis.Generators.Tests/         # Generator + analyzer tests (net10.0)
...
Prognosis  ←──  Prognosis.DependencyInjection
    ...
    │               (+ System.Reactive)
    │
    └───────  Prognosis.Generators  (analyzer reference only)
...
- `HealthNode.Create(name)` — single entry point; intrinsic status is always `Healthy`
- `.WithHealthProbe(healthCheck)` — fluent method to attach a custom intrinsic health check delegate
- `HealthNode.CreateDelegate(name, healthCheck)` — **deprecated**, use `Create(name).WithHealthProbe(healthCheck)`
- `HealthNode.CreateDelegate(name)` — **deprecated**, use `Create(name)`
- `HealthNode.CreateComposite(name)` — **deprecated**, use `Create(name)`
...
services.AddPrognosis(health => {
    health.AddDiscoveredNodes();          // auto-generated by Prognosis.Generators
    health.AddNode("ExternalService")
        .WithHealthProbe<ExternalService>(...);
    health.AddNode("Aggregator")
        .DependsOn(...);
...
1. **Service node registration** — `AddServiceNode<T>` (called by generated `AddDiscoveredNodes()` or manually) stores a selector `Func<T, HealthNode>` and optional edge definitions. Services are registered as singletons.
2. **Node pool** — built lazily on first graph resolution. A shared `Dictionary<string, HealthNode>` holds all nodes from service nodes and `AddNode` definitions.
3. **Edge wiring** — `[DependsOn]` attribute-declared and configurator-declared edges are resolved by name against the pool and wired via `DependsOn()`.
...
| Type | Responsibility |
|---|---|
| `ServiceCollectionExtensions` | `AddPrognosis()` entry point. Orchestrates service node registration, pool building, root resolution. |
| `PrognosisBuilder` | Fluent builder collecting service node definitions, node definitions (via `AddNode`), and root declarations. |
| `NodeConfigurator` | Fluent node definition API: `WithHealthProbe<T>()`, `DependsOn()`. Returned by `PrognosisBuilder.AddNode()`. |
| `DependsOnAttribute` | Property-level attribute: `[DependsOn("Database", Importance.Required)]` on `HealthNode` properties. Read by the source generator. |
...
## Extension: Source Generators (`Prognosis.Generators`)

A Roslyn source generator and diagnostic analyzer, packaged as an analyzer reference (no runtime dependency added).

### HealthNodeNameCollector (Incremental Generator)

Scans every `HealthNode.Create("name")`, `HealthNode.CreateDelegate("name", ...)`, and `HealthNode.CreateComposite("name")` invocation in the current compilation. Extracts string literal and `const` arguments via the semantic model. Emits `HealthNames.g.cs` with one `const string` field per discovered name, deduped and sorted.

Dotted names are sanitized to valid identifiers (`"Database.Connection"` → `Database_Connection`). Leading digits are prefixed with `_`.

### DependsOnEdgeAnalyzer (Diagnostic Analyzer)

Validates string arguments in `DependencyConfigurator.DependsOn(string, Importance)` calls against the set of names discovered by the collector. Reports `PROGNOSIS001` (warning) for unrecognized names. Uses `CompilationStart` + `CompilationEnd` pattern: a `SyntaxNodeAction` collects both node definitions and pending references concurrently, then cross-references at compilation end.

### ServiceNodeDiscoveryGenerator (Incremental Generator)

Scans for classes with public `HealthNode` properties, reads `[DependsOn("name", Importance)]` attributes on those properties, and emits an `AddDiscoveredNodes()` extension method on `PrognosisBuilder`. Each discovered property becomes an `AddServiceNode<T>(svc => svc.PropertyName, ...)` call with attribute-declared edges wired automatically.

Only emits when `Prognosis.DependencyInjection.PrognosisBuilder` is referenceable in the compilation (i.e., the project references the DI package). Projects that reference only the core `Prognosis` package get `HealthNames` but not `AddDiscoveredNodes`.

**Scope limitation:** The generators and analyzer see only the current compilation's syntax trees. Nodes created at runtime by the DI builder (`AddDelegate`, `AddComposite`) or in referenced assemblies are not visible. For those names, use hand-written `const` fields in a shared library.
...
| [002 — Remove `HealthNode.Evaluate` (cache-only model)](adr/002-remove-evaluate-cache-only-model.md) | Dropped the "quiet" recursive `Evaluate()` primitive, collapsing to a single evaluation path with three operations: `GetReport()` (read cache), `Refresh()` (point change → propagate → notify), and `RefreshAll()` (full bottom-up DFS poll). |
| [003 — Source-generated service discovery](adr/003-replace-ihealthaware-with-generator-discovery.md) | Replaced the `IHealthAware` marker interface and reflection-based `ScanForServices` assembly scanning with source-generated discovery and `[DependsOn<T>]` wiring — zero-reflection, AOT/trim-safe, compile-time-checked DI registration. |
| [004 — Split composite nodes and health probes](adr/004-split-composite-nodes-and-health-probes.md) _(Proposed)_ | Separate a node's two roles — intrinsic health probe vs. composite aggregator — into distinct concepts, removing the dual-modeling ambiguity where `WithHealthProbe` + `DependsOn` on one node expressed the same graph two ways. |
| [005 — Node tags](adr/005-node-tags.md) | Added immutable string key/value `Tags` to `HealthNode`, carried through `HealthSnapshot` / `HealthTreeSnapshot`, instead of a parallel `RichHealthReport` type — enriching existing snapshots without splitting the API or breaking `DiffTo`. |
| [006 — `Unknown` is strictly non-gating](adr/006-unknown-non-gating-rollup.md) | Pinned, as a named test-guarded invariant, that an `Unknown` dependency (any `Importance`) raises its parent at most to `Unknown` — never `Degraded`/`Unhealthy` — so startup gates and run-signals can rely on the not-yet-probed state. |
| [007 — Counterfactual & contributor analysis](adr/007-counterfactual-contributor-analysis.md) | Add a pure, non-mutating `Prognosis.Diagnostics` layer over `HealthTreeSnapshot` — `WhatIf` (counterfactual re-fold), `Contributors` (determining leaves), `MinimalHealingSet` — to answer which nodes gate a rollup and whether repairing one would heal it, backed by a single extracted fold helper. |
| [008 — `Unknown` is transient by contract; `Importance.Advisory`](adr/008-unknown-must-be-transient.md) | Forbade a node *resting* at `Unknown` — the state means "not yet determined" and must carry a resolution path — and added `Importance.Advisory` for edges to probes whose signal may be structurally absent, where `Unknown` is absorbed rather than propagated. Also pinned the no-silent-default guard on every `Importance` / `HealthStatus` switch. |
| [009 — Topology is a first-class cached artifact](adr/009-topology-first-class-enrich-projection.md) _(Proposed)_ | Split structure from status: `HealthTopology` ships once per `TopologyChanged`, flat reports ship per beat, and `HealthGraphAnalysis.BuildTreeSnapshot` recombines them — making the tree a projection of the report rather than a live walk, and giving reactive consumers a safe entry point into the diagnostic layer. |
| [010 — Leased verdicts (TTL decay)](adr/010-leased-verdicts-ttl-decay.md) _(Proposed)_ | Opt-in TTL decay for push-fed nodes: a lease whose producer stops affirming decays through a bounded `Unknown` window (`Ttl` → `Ttl + EscalateAfter`) and on to `Unhealthy`, with band-quantized reason text so the decay itself does not churn the report stream. |
| [011 — Temporal policy pipeline](adr/011-temporal-policy-pipeline.md) _(Proposed)_ | Moved grace and debounce into the node as a fixed, library-owned chain applied at one choke point; derived flap as an observation rather than stored state; added graph-wide `TemporalDefaults` so one bag replaces N per-node registrations. |
| [012 — The report-equality contract](adr/012-report-equality-contract.md) _(Proposed)_ | Pinned the report-equality key to `(Name, Status, Reason)` per node — `Tags` and `Temporal` excluded — kept the transition stream `Status`-keyed, and made `Reason` content normative: an explanation, stable between meaningful changes, never a telemetry channel. Amended so the rule binds the *aggregator* that composes reasons, not only the probe that emits one. |
| [013 — Structured temporal-state field](adr/013-structured-temporal-state-field.md) _(Proposed)_ | Added a sparse `TemporalState? Temporal` to `HealthSnapshot` carrying lease staleness, windowed flap, policy phase and pending deadline — on the wire for point-in-time readers, but excluded from report-change detection so continuously-varying data cannot reintroduce per-wave churn. |
