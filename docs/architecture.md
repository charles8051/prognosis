...
services.AddPrognosis(health => {
    ...
    health.AddProbe<ExternalService>(...);
...
2. **Node pool** — built lazily on first graph resolution. A shared `Dictionary<string, HealthNode>` holds all nodes from service nodes, composites, and probes.
...
| `PrognosisBuilder` | Fluent builder collecting service node definitions, composite definitions, probe definitions, and root declarations. |
| `DependencyConfigurator` | Fluent edge declaration API used inside `AddComposite`, `AddDelegate`, and `AddServiceNode` callbacks. |
