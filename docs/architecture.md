...
2. **Node pool** — built lazily on first graph resolution. A shared `Dictionary<string, HealthNode>` holds all nodes from service nodes, composites, and delegates.
...
| `PrognosisBuilder` | Fluent builder collecting service node definitions, composite definitions, delegate definitions, and root declarations. |
...
Scans every `HealthNode.CreateDelegate("name", ...)` and `HealthNode.CreateComposite("name")` invocation in the current compilation. Extracts string literal and `const` arguments via the semantic model. Emits `HealthNames.g.cs` with one `const string` field per discovered name, deduped and sorted.
